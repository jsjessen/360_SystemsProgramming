...
// It tries to read nbyte from fd to buf[ ], and returns the 
...
//mywrite behaves exactly the same as Unix's write(fd, buf, nbyte) syscall.
//It writes nbyte from buf[ ] to the file descriptor fd, extending the file 
//size as needed.
